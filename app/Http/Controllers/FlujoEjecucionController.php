...
class FlujoEjecucionController extends Controller
{
    ...
    public function show(Flujo $flujo, FlujoEjecucion $ejecucion): JsonResponse
    {
        ...
        $etapasConEstadisticas = $ejecucion->etapas->map(function ($etapa) use ($enviosPorEtapa, $prospectosAlcanzados) {
            ...
            return [
                ...
                'primer_envio_at' => $etapa->primer_envio_at,
    ...
    public function cohortProspectos(CohortProspectosRequest $request, Flujo $flujo, FlujoEjecucion $ejecucion): JsonResponse
    {
        try {
            ...
            // For large arrays (>1000), use whereIn with chunking
            // For smaller arrays, whereIn is efficient
            $query = Prospecto::query()->whereIn('id', $prospectosIds);
            ...
            Log::error('cohortProspectos: Error', [
                'flujo_id' => $flujo->id,
                'ejecucion_id' => $ejecucion->id,
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ...
            return response()->json([
                'error' => true,
                'mensaje' => 'Error al obtener prospectos de la cohorte',
                'detalle' => $e->getMessage(),
            ], 500);
        }
    }
